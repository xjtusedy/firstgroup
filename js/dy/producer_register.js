  //用户名：  
        ...
        function validateForm(){ //定义validateForm方法用于客户端校验  
            var flag = true;  
            //校验用户名  
            var usernameNode = byId("producername"); //获得ID值为username的节点对象  
            var username = usernameNode.value;   //获得usernameNode节点的值，即用户在username文本框内填写的值  
            if(!usernameRegex.test(username)){    //验证获得到的值是否符合正则表达式  
                byId("producername_span").style.color = "red"; //如果不符合，则将ID值为username_span的节点对象内容变为红色  
        ...
        function checkUsername(node){ //当鼠标离开节点时调用此方法，验证节点内容是否符合注册规范  
            ...
            var producername = node.value;  //得到传递过来的节点对象的值
            if(producername.length<2||producername.length>10){  //验证是否符合节点对应的正则表达式  
            	
            	byId("producer_span").style.color = "red"; //不符合，相应内容变成红色  
            	byId("form-validate").style.opacity = "1";
            }else{  
            	byId("producer_span").style.color = "green";  //符合，相应内容变成绿色  
